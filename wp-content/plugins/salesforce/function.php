<?php
function writeToLog($e,$line,$file) 
 {

	$timezone=date_default_timezone_get();
	$mailto=array('nishu.developtech@gmail.com','dt.amanjotkaur@gmail.com');
	...
	{
		$mystring = 'DateTime: '.$date_time.' '.$timezone.','. PHP_EOL .'Error Message: '.$e.','. PHP_EOL .'Line: '.$line.',File Name:'.$file .PHP_EOL .PHP_EOL;
		fwrite( $h, $mystring );
		fclose($h);
		$mailSubject='Wineoh error';
		$mailBody=$mystring;
		foreach($mailto as $values)
		{
			  send_email($values,$mailSubject,$mailBody);
		}
  }
  else
...
function Zendesk_Api_get_user_id($email)
{
 $zendesk_id = zendesk ("/api/v2/users/search.json?query=".$email,"GET",array());
 return $zendesk_id;
}
...
function full_contact_api($email,$apikey,$contact_type,$account_id,$fname,$lname,$password)
{
		...
		$add_new_contact->YouTube_URL__c= $youtube_url;
		$add_new_contact->Youtube_Username__c= $c_youtube_username;
		$add_new_contact->Youtube_Bio__c= $c_youtube_bio;
		if($c_youtube_followers!=0)
		{
			$add_new_contact->Youtube_Followers__c= $c_youtube_followers;
		}
		$add_new_contact->AngelList__c= $c_angellist_url;
		$add_new_contact->AngelList_Username__c= $c_angellist_username;
		$add_new_contact->AngelList_Bio__c= $c_angellist_bio;
		if($c_angellist_followers!=0)
		{
			$add_new_contact->AngelList_Followers__c= $c_angellist_followers;
		}
		
		$add_new_contact->GooglePlus_URL__c= $c_google_url;
		$add_new_contact->GooglePlus_Username__c= $c_google_username;
		$add_new_contact->GooglePlus_Bio__c= $c_google_bio;
		if($c_google_followers!=0)
		{
			$add_new_contact->GooglePlus_Followers__c= $c_google_followers;
		}	
		
		$add_new_contact->Instagram_URL__c= $c_instagram_url;
		$add_new_contact->Instagram_Username__c= $c_instagram_username;
		$add_new_contact->Instagram_Bio__c= $c_instagram_bio;
		if($c_instagram_followers!=0)
		{
			$add_new_contact->Instagram_Followers__c= $c_instagram_followers;
		}
		
		$add_new_contact->LinkedIn_URL__c= $c_linkedin_url;
		$add_new_contact->linkedin_username__c= $c_linkedin_username;
		$add_new_contact->LinkedIn_Bio__c= $c_linkedin_bio;
		if($c_linkedin_followers!=0)
		{
			$add_new_contact->linkedIn_followers__c= $c_linkedin_followers;
		}
		
		$add_new_contact->Pinterest_URL__c= $c_pinterest_url;
		$add_new_contact->Pinterest_username_C__c= $c_pinterest_username;
		$add_new_contact->Pinterest_Bio__c= $c_pinterest_bio;
		if($c_pinterest_followers!=0)
		{
			$add_new_contact->Pinterest_Followers__c= $c_pinterest_followers;
		}
		
		$add_new_contact->Twitter_URL__c= $c_twitter_url;
		$add_new_contact->Twitter_Username__c= $c_twitter_username;
		$add_new_contact->Twitter_Bio__c= $c_twitter_bio;
		if($c_twitter_followers!=0)
		{
			$add_new_contact->Twitter_Followers__c= $c_twitter_followers;
		}
		
		
		return $add_new_contact;
}
function Okta_Api($first_name,$last_name,$email,$password,$security_question,$awnser)
{
...
function update_okta_status_zendesk_id($user_id,$okta_status,$zendesk_id)
{
	$update_user_profile = array
				(
					"profile" => array
								(
									"status_okta"=> $okta_status,
									"id_zendesk"=> $zendesk_id	
								)
				);

	$user_profile_result = Okta ("/api/v1/users/".$user_id, "POST", $update_user_profile);
...
function set_okta_zendesk_account($email,$first_name,$last_name,$password,$contact_type,$security_question,$awnser)
{
	$baseUrl = "https://dev-817806.oktapreview.com";
	...
	/*OCTA Api Starts*/
	$add_user_result=Okta_Api($first_name,$last_name,$email,$password,$security_question,$awnser);

	if(isset($add_user_result->errorCauses))
	{									
		$msg['error']=$add_user_result->errorCauses[0]->errorSummary;
		writeToLog($msg['error'],__LINE__,__FILE__);
	}
	else
	{
		/* Zendesk API Start*/
		...
		{
			$okta_status=$add_user_result->status;
	...
	if(isset($msg['id']))
	{
		if(trim($msg['id'])!=null)
		{
			...
			$mailBody='<!DOCTYPE html>
						...
						<br/>
						Click the following <a href='.home_url().'/send-verification-email?tokenT='.$contact_type.'&tokenI='.$msg['id'].'> link </a> to activate your account. This link expires in 7 days.<br/>
						<br/>
			...
			$zendesk_result=Zendesk_Api_get_user_id($email);	

			if(isset($zendesk_result->error))
			{
				if(isset($zendesk_result->error->title) && isset($zendesk_result->error->message))
				{
					$msg['error']= $zendesk_result->error->title ."<br/>";
					$msg['error'].= $zendesk_result->error->message;
				}
				else
				{
					$msg['error']= $zendesk_result->error ."<br/>";
					$msg['error'].= $zendesk_result->description;
				}
				writeToLog($msg['error'],__LINE__,__FILE__);
			}
			else
			{
				  $zendesk_id=$zendesk_result->users[0]->id;

				 update_okta_status_zendesk_id($msg['id'],$okta_status,$zendesk_id);
...
function assign_zendesk_app($email,$user_id)
{
 ...
 $add_user_result = Okta ("/api/v1/apps/0oacabrprihMHr3Zy0h7/users", "POST", $user_login);
 return $add_user_result;
}
